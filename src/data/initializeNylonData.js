...
export function useNylonData() {
  ...
  React.useEffect(() => {
    ...
    const resetBaseline = (votes) => {
      baselineRef.current = {
        time: Date.now(),
        votes,
      }
      console.log('Baseline reset at', new Date(baselineRef.current.time).toLocaleTimeString())
    }
    ...
    const startPolling = () => {
      ...
      if (!intervals.htmlParse) {
        intervals.htmlParse = setInterval(async () => {
          const votes = await fetchVotes(setState, lastVoteSnapshotRef, baselineRef)
          if (!baselineRef.current.votes) {
            resetBaseline(votes) // first fetch sets baseline
          }
        }, 1000 * 3)
      }
      if (!intervals.baselineChecker) {
        intervals.baselineChecker = setInterval(() => {
          const TWO_MIN = 1000 * 60 * 2

          // How much time has passed since last baseline
          const elapsed = Date.now() - baselineRef.current.time
          const remaining = Math.max(0, Math.floor((TWO_MIN - elapsed) / 1000))

          // Update Zustand countdown every second
          useVoteStore.getState().setCountdown(remaining)

          // If 2 minutes have passed → reset baseline
          if (elapsed >= TWO_MIN) {
            if (lastVoteSnapshotRef.current) {
              resetBaseline(lastVoteSnapshotRef.current)
            }
          }
        }, 1000) // still runs every second
      }
    }
...
async function fetchVotes(setState, lastVoteSnapshotRef, baselineRef) {
  try {
    const response = await fetch('https://polls.polldaddy.com/vote-js.php?p=15909793')
    const text = await response.text()
    const pollData = extractPollData(text)

    // Save last snapshot for later comparisons
    lastVoteSnapshotRef.current = pollData

    processVotes(pollData, setState, lastVoteSnapshotRef, baselineRef)

    return pollData // allow baseline reset to use it
  } catch (err) {
...
function processVotes(votes, stateSetter, lastVotesSnapshot, baselineSnapshot) {
  const lastVotesSnapshotCurrent = lastVotesSnapshot.current
  const baselineSnapshotCurrent = baselineSnapshot.current
  ...
  const allParticipants = Object.entries(votes)
    .map(([name, count]) => {
      // Try to find the candidate by name
      ...
      if (lastVotesSnapshotCurrent) {
        const prevCount = lastVotesSnapshotCurrent[name]
        if (typeof prevCount === 'number') {
          deltaFromLastSnapshot = Math.max(count - prevCount, 0)
        }
      }

      // Delta vs baseline
      let deltaFromBaseline = null
      const baselineSnapshotCurrent = baselineSnapshot.current // ✅ unwrap
      if (baselineSnapshotCurrent && baselineSnapshotCurrent.votes) {
        const baseCount = baselineSnapshotCurrent.votes[name]
        if (typeof baseCount === 'number') {
          deltaFromBaseline = count - baseCount
        }
      }
  ...
  stateSetter({
    ...
    baselineTime: convertToPhTime(baselineSnapshotCurrent.time),
  })
}
;`1`
